...
class ReactionHandler(
    ...
    override suspend fun handle(
        ...
        if (emoji == null && type != LIKE_TYPE) {
            logger.info("$type に絵文字が無いので何もしない: ${recipient.acct} ← $verifiedSignerActorId")
            return
        }
        ...
        if (undoneFirst) {
            logger.info("先に取り消しが届いた $type なので記録しない: ${recipient.acct} ← $verifiedSignerActorId id=$activityUri")
            return
        }
    ...
    private fun addFavourite(
        ...
        if (recorded) {
            logger.info("お気に入りを記録した: ${recipient.acct} ← ${actor.actorId} 投稿=${notePublicId.value}")
        } else {
            logger.info("お気に入りを記録しなかった。記録済みか投稿が無い: ${recipient.acct} ← ${actor.actorId} 投稿=${notePublicId.value}")
        }
    }
    ...
    private fun putStamp(
        ...
        if (recorded) {
            logger.info("スタンプを記録した: ${recipient.acct} ← ${actor.actorId} 投稿=${notePublicId.value} 絵文字=$emoji")
        } else {
            logger.info("スタンプを記録しなかった。投稿が無い: ${recipient.acct} ← ${actor.actorId} 投稿=${notePublicId.value}")
        }
    }
