...
interface FeedRepository {
    ...
    /**
     * 次の取得予定が [now] を過ぎている、登録時の取り込みが済んだフィードを返す。
     *
     * 定期ポーリングの対象を選ぶためのもの。一度に取りに行く数を [limit] で抑える。
     *
     * 取り込みが済んでいないものを含めない。登録の途中に取りに行くと、既存記事を
     * こちらが先に保存して新着として扱ってしまう。[limit] より先に外すのは、
     * 登録が中断されたフィードが残ったときに、それだけで毎回の枠が埋まらないようにするため。
     */
    fun findDue(
        now: Instant,
        limit: Int,
