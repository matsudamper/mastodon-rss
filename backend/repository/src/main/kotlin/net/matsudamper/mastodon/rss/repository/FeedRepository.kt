...
interface FeedRepository {
     ...
     *
     * 登録時の取り込みが済んでいないものは、登録から取得の間隔が過ぎたものだけ返す。
     * 登録の途中に取りに行くと、既存記事をこちらが先に保存して新着として扱ってしまう。
     * 間隔を過ぎても済んでいなければ登録は終わっていないので、取り込みからやり直させる。
     */
    fun findDue(
        now: Instant,
        limit: Int,
     ...
     *
     * 登録は保存と取り込みが別々に確定するので、途中で終わったものを登録し直す経路がある。
     * 消してから入れられないと分かると、記事ごと失って再登録もできなくなる。
     *
     * @return 入れ替えた後のフィード。[existingId] が無い、他のアカウントか他のフィードが
     *   同じ URL を使っている場合は、何も変えずに null
     */
    fun replace(
        existingId: FeedId,
        feed: NewFeed,
