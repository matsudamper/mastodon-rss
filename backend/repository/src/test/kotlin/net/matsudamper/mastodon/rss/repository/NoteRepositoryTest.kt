...
class NoteRepositoryTest {
    ...
    fun `新しい順に返り、cursor で続きから取れる`() {
        withRepository { notes ->
            ...
            assertEquals(listOf("note-4", "note-3"), first.map { it.publicId })

            val second = notes.list("admin", after = first.last().cursor(), limit = 2)
            assertEquals(listOf("note-2", "note-1"), second.map { it.publicId })

            val third = notes.list("admin", after = second.last().cursor(), limit = 2)
            assertEquals(listOf("note-0"), third.map { it.publicId })
    ...
    fun `途中に投稿が増えても cursor なら取りこぼさない`() {
        withRepository { notes ->
            ...
            val first = notes.list("admin", after = null, limit = 2)
            assertEquals(listOf("note-3", "note-2"), first.map { it.publicId })
            ...
            assertEquals(
                ...
                notes.list("admin", after = first.last().cursor(), limit = 2).map { it.publicId },
            )
        }
    }
    ...
    fun `同じ時刻でも並びが決まる`() {
        withRepository { notes ->
            ...
            assertEquals(
                ...
                notes.list("admin", after = null, limit = 10).map { it.publicId },
            )
        }
    }
    ...
    fun `アカウントごとに分かれ、名前の大文字小文字は区別しない`() {
        withRepository { notes ->
            ...
            assertEquals(listOf("b"), notes.list("feed1", after = null, limit = 10).map { it.publicId })
        }
    }
    ...
    fun `開き直しても投稿が残っている`() {
        ...
        withRepository { notes ->
            assertEquals("<p>abc</p>", notes.find("abc")?.contentHtml)
            assertEquals(1, notes.count("admin"))
        }
    }
}
