...
入口の `index.html` はキャッシュさせない（`Cache-Control: no-store`）。名前に中身のハッシュが
入っているものは `immutable` で 1 年持たせる。配布物では JS と `.wasm` がそれにあたる。

判断するのは拡張子ではなく名前。置き場所（`STATIC_SRC_DIR`）にはビルドした配布物以外も
入るので、拡張子で決めると、古い配布物の `frontend.js` を置いたときに 1 年間差し替えられなくなる。
