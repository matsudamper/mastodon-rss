...
`:frontend:common-component` は画面部品のうち、このアプリの画面や API を知らないもの
だけを置く。パスワード入力のような HTML の input を載せる部品は、別の Compose Web
アプリからも使えるのでここに置く。管理画面ログインの文言や form の id は
`:frontend` に残す。
...
`TransparentScreenSceneStrategy` が下の画面を描いたまま重ねる。こうすると URL が付いて
直接開けるし、閉じるのが戻ると同じ操作になり、その操作の間だけの状態を別の ViewModel に
置ける。

metadata が持つのは重なるという 1 点だけで、ダイアログにするかシートにするかは画面側が
決める。閉じてよいかは操作の途中かどうかで変わるので、metadata に固定で置くと表せない。
下の画面は描かれ続けるだけで止まらないため、入力を遮るのも画面側の責任
（`AlertDialog` の暗幕がその役目をする）。
...
こちらから相手の inbox に送るものはその場で送らず `delivery_queue` に投函して、
`:backend` のワーカー（`DeliveryWorker`）が送る。送信中にプロセスが落ちても
投函した行が残るので、次の起動で送り直せる。
...
行の形は「署名するアカウント・宛先・送るボディ」と種別（`kind`）なので、種別を増やしても
テーブルは変わらない。`Create{Note}` 以外は `:backend:feature-mastodon` の `ActivityQueue`
から投函する。組み立てた側は送り先と中身だけを渡し、いつ送るかは持たない。

- `Accept{Follow}`: `FollowHandler` が投函する。投函できた時点でフォロワーとして数える。
  相手が受け取ったかどうかは待たない。待つと inbox の応答が相手のタイムアウトに掛かる
- フォロー成立後の過去の投稿（`Create{Note}`）: `Accept` の後に投函する。同じホスト宛は
  投函した順に 1 件ずつ送られるので、`Accept` より先に `Create` が届くことはない。
  行は投稿に繋ぐので、送る前に投稿を消せば行ごと消える
- `Delete{Note}`: 投稿の記録を消してから投函する。行は投稿に繋がない（繋ぐ先が無い）
- `Delete{Actor}`: アカウントの行を消してから投函する。署名するアクターを引き当てで
  引けないので、ワーカーはこの種別だけ名前から URL を組み立てる
- `Update{Actor}`: プロフィールと画像の更新で投函する
