...
1 トランザクションで行う。同じホスト宛は 1 件ずつ直列、異なるホストは並列で、1 回の
claim（8 件）が同時実行数の上限になる。送り直しの間隔は `DeliveryRetryPolicy` にあり、
30 秒から 2 倍ずつ、24 時間で頭打ち、投函から 30 日で諦める。諦めた行は `failed` のまま
残り、`body` だけ消す。
