...
記事から作った投稿には、リンク先のページの `og:image` を `attachment` に 1 枚だけ入れる。
`type` は `Image` で、`mediaType` は入れない。こちらは URL を知っているだけで、
中身が何かは取りに行くまで分からないため。Mastodon は添付が付いた投稿には
リンクのプレビューカードを出さないので、相手に作らせる代わりにこちらが選んだ 1 枚を出す形になる。
配信する `Create` と、パーマリンクと `outbox` の 3 経路とも同じ形で返す。
