...
お気に入りは Mastodon と同じく、投稿と押した相手の組で 1 つとして記録する。`Like` の
id は持たないので、取り消しの `Undo` は `object` に `Like` が丸ごと埋まっているときだけ
引き当てられる。id だけで指してくる `Undo` は、Mastodon と同じくお気に入りの取り消し
としては扱わない。`Like` より先に `Undo` が届いたときは、その id を 24 時間覚えておき、
後から届いた `Like` を記録しない。これも Mastodon と同じ。
