...
`/users/{name}/icon` は無認証なので、繋ぐ前に名前を引いて、ループバックや
プライベートアドレスを指すものは取りに行かない。リダイレクトも自分で辿って
飛び先を同じように見る。

取ってきた中身は `ICON_CACHE_DIR` に置き、置き場と期限を `feed_icons` に入れる。
期限は配信元の `Cache-Control` の `max-age` に従い、言ってこなければ 1 時間、
長くても 1 日で切る。期限が切れるまでは置いてあるものを返すので、
叩かれた数だけ配信元へ出ていくことはない。
返すときの `Cache-Control` にも同じ期限を出す。ここだけ長く持たせると、
こちらが取り直した後も見に来た側には古いものが出続ける。
取り直せなかったときは、期限が切れていても置いてあるものを返す。
配信元が落ちている間だけアイコンが消えると、見ている側からは壊れて見える。
