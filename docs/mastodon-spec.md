...
管理画面の `admin.deleteAccount` で消す。フォロワーと配信した投稿を消してから、
`object` に自分のアクター id を入れた `Delete` を、消える直前のフォロワーの inbox に配る。
`object` が送り主自身でないと、相手はアカウントではなく投稿の削除として扱う。

配らないと相手側にアクターとフォローの関係が残り、同じ名前で作り直したときに
フォローしていない相手に投稿が届いたように見える。届かなかった相手には残る。

こちらの記録は投稿の削除と同じく配る前に消す。フィードと取り込んだ記事は
アカウントの行を消すと外部キーで一緒に消えるので、消した後は同じフィードを
最初から登録し直せる。
