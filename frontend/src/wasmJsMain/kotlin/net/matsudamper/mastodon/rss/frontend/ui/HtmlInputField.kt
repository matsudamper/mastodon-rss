...
/**
 * ブラウザの input を Compose 上に重ねる TextField。
 *
 * canvas 上の [androidx.compose.material3.OutlinedTextField] ではブラウザの
 * オートフィルやクリップボード操作が期待どおり動かないため、HTML の input を埋め込む。
...
/**
 * 画面外に置く HTML form。オートフィルと Enter 送信のため。
 *
 * [HtmlInputField] の `formId` と同じ id を渡す。
 */
@Composable
fun HiddenHtmlForm(
    formId: String,
