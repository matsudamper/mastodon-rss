...
サーバーは `app`（uid 10001）として動くが、コンテナ自体は root で起動する。
entrypoint が `/data` の所有者を `app` に合わせてから `setpriv` で権限を落とす。
イメージ側で `chown` しても、そこにボリュームを被せると隠れてしまい、所有者が
伝わるのは Docker の copy-up だけになる。これはボリュームが新規で空のときの 1 回しか
走らず、バインドマウントでは走らない。環境によっては新品のボリュームでも root 所有に
なり、`/data/actor-private-key.pem` の生成が `AccessDeniedException` で落ちる。
起動のたびに合わせておけば、ボリュームでもバインドマウントでも同じように動く。
