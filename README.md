...
`/data` をホストのディレクトリに向けると、そのディレクトリごと uid 10001 の持ち物になる。
ホストのユーザーからは書けなくなるので、`scp` で DB を送り込むと Permission denied になる。

ホスト側でも読み書きするなら、`APP_UID` と `APP_GID` にそのユーザーの id を渡す。
entrypoint はその id で `/data` を chown し、サーバーもその id で動く。
...
`.env` に書けば `docker compose` がそのまま渡す。既に 10001 の持ち物になっている
ディレクトリも、次の起動時に指定した id へ chown される。
