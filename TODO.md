...
次の一手: Phase 6（フィードとアクターを 1:1 で紐付け、アクターごとに鍵を持たせる）。
...
- Phase 5（RSS の取り込みと自動投稿）のチェックポイントまで完了。実機で RSS を登録して
  放置し、新着記事が自動でタイムラインに流れることを確認した。
  ActivityPub 側と独立していて後戻りが出ないため前倒しした。
  条件付き GET と、取り込み失敗を管理画面から見る口は残っている（Phase 5 の項目を参照）
...
- [x] `Follow` を記録して `Accept` を投函する
      - 記録と投函は 1 トランザクション。フォロワーとして数え始めるのは `Accept` を
        送れたときで、送れるまで配信キューが送り直す
- [x] `Undo{Follow}` を処理してフォロー解除
- [x] `Delete{Actor}`（アカウント削除・引っ越し）を処理してフォロワーを掃除
      - [ ] フォローの記録が無い相手からの `Delete` は検証できず 202 で受け流すだけ。
            掃除する対象も無いので、そのままにしてある
...
- [x] 配信キュー: 失敗時に指数バックオフでリトライ、上限到達で諦める
      （仕組みは [docs/architecture.md](docs/architecture.md) の「配信キュー」）
      - [ ] 諦めた行（`failed`）は消えない。掃除は Phase 7 の到達不能インスタンス対応と一緒に決める
      - [ ] 送っている最中の投稿を消すと、相手には `Delete` の後に `Create` が届きうる
      - [ ] 投函の直前にアカウントを消して同じ名前で作り直すと、消しそこねた行が
            新しいアカウントとして配られうる。行はアカウントを名前で持っている
      - [ ] フォロー成立後に過去の投稿を配るのだけはキューに載せず、その場で送る。
            載せると、`Follow` を並べて送るだけで行を好きなだけ積ませられる
      - [ ] 投函した後にフォローを解除されても行は残る。解除済みの inbox へ、成功するか
            期限が来るまで送り続ける（未送信の `Accept` だけは解除で消える）
      - [ ] 消したアカウントの行は、配信を送り切るまで残る。諦めた行（`failed`）が
            残っている間はその名前を作り直せない。掃除は起動時だけ
      - [ ] 宛先の確定と投函の間にフォローが成立すると、その 1 件は通常配信にも backfill にも
            入らず届かない
      - [ ] 実機で確認する
...
実機で登録できることを確認した。登録時は既存記事を保存せず、定期ポーリングで取り込んで投稿する。
チェックポイントは達成済みで、残りは配信元に優しくする工夫と管理画面からの見え方。
どこまでやったかは各項目の下に書いた。
...
- [x] スケジューラ（定期ポーリング）。フィードごとに間隔を設定可能に
      - 間隔を管理画面から変える口はまだ無い
- [x] 初回登録時は既存記事を保存せず、件数と最新 1 件をプレビューできる
      - 登録前からあった記事は最初のポーリングで取り込んで投稿する
...
- [ ] アクター情報更新時に `Update{Actor}` を配信（アイコン・説明文の変更を伝播させる）
      - 表示名と説明文の編集時と、取り込みでアイコン / ヘッダー画像が入れ替わったときは配信する。
        `attachment` の変更時は未実装
- [x] アイコン / ヘッダー画像（`icon` / `image`）の配信
- [ ] フィードアクターのプロフィールに `admin` へのリンクを置く
      ...
      - フィードのサイトと配信元 URL は `attachment` に出るようにした。
        `admin` へのリンクは未着手。`attachment` 変更時の `Update{Actor}` 配信も未実装
...
- [ ] 配信の並列化とインスタンス単位のレート制限
      - 異なるホスト宛を並列に送るところまでは Phase 4 の配信キューで入っている。
        1 つのホストに対する送信の間隔を空けるのは未着手
...
- [ ] 未対応アクティビティ（`Announce`, `Create{Note}` の返信など）を安全に無視する
      - `Like` と `EmojiReact`（Misskey / Pleroma のスタンプ）は受け取って記録するようにした。
        取り消しの `Undo` と、押した相手が消えたときの掃除も入れている
...
- [ ] Compose でフィード一覧 / 追加 / 削除
      - フィードの追加は `/admin/accounts/@{name}/feeds/new` のダイアログに入れた。
        ...
        フィード自体の削除は未実装
- [x] プロフィール（表示名・説明文）の編集
- [x] 公開画面に届いたお気に入りとスタンプを出す
      - 投稿の一覧と投稿 1 件の画面に、絵文字と押された数を並べる。押した相手は出さない
- [ ] アクターごとのフォロワー数・最終投稿・配信エラーの表示
      - フォロワー数と配信キューの状態（待ち件数・諦めた件数・送り直し待ちと諦めた宛先と
        その種別）は `/admin/accounts/@{name}` に出している。最終投稿は未着手
      - アカウントを問わない送り直し待ちは `/admin/deliveries` に出している。
        まだ一度も送っていない分と諦めた分はアカウントごとの画面にしか出ない
