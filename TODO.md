...
- [ ] アクター情報更新時に `Update{Actor}` を配信（アイコン・説明文の変更を伝播させる）
      - 表示名と説明文は管理画面から編集して `accounts.display_name` / `accounts.summary` に保存し、
        Actor の `name` / `summary` に出る。既存フォロワーへの `Update{Actor}` 配信は未実装
...
- [x] プロフィール（表示名・説明文）の編集
      - `/admin/accounts/@{name}/profile` のダイアログで編集する。登録済みフィードの題名と説明も反映できる
