...
- [x] `Follow` を記録してから `Accept` を返す
      - [ ] `Accept` を返した後の記録に何度も失敗した場合は諦めてログに残すだけ。
            ...
            配信キューは入ったが載せているのは投稿だけで、`Accept` は今まで通りその場で送る。
            キューの行は種別（`kind`）を持つので、載せるときにテーブルを作り直す必要は無い
...
- [x] 配信キュー: 失敗時に指数バックオフでリトライ、上限到達で諦める
      - `delivery_queue` に 1 宛先 1 行で投函し、ワーカーが claim して送る。
        30 秒から 2 倍ずつ、24 時間で頭打ち、投函から 30 日で諦める。
        投稿の記録・記事の投稿済み化・投函は 1 トランザクション
      - [ ] 諦めた行（`failed`）は消えない。掃除は Phase 7 の到達不能インスタンス対応と一緒に決める
      - [ ] 1 回の claim が同じホスト宛の行で埋まると、その分だけ他のホスト宛が待つ。
            ホスト単位の取り出しの制限は Phase 7 の「配信の並列化とインスタンス単位のレート制限」で入れる
      - [ ] 送っている最中の投稿を消すと、相手には `Delete` の後に `Create` が届きうる。
            送る直前までの取り消しは行の削除で効くが、送信中の 1 件は止められない
