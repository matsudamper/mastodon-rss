...
Phase 2 は送信側まで実装した。inbox が受け取ったアクティビティの署名を検証し、
`Follow` なら相手の inbox に署名付きで `Accept` を返す。フォロワーの記録はまだしない。
...
実装は揃ったが、実機での確認はまだ。`test-` のアカウントで一度フォローして、
「フォロー中」で確定することを見る必要がある。手元のテストで見ているのは
自分で署名して自分で検証するところまでで、Mastodon が受け取るかどうかは別。
