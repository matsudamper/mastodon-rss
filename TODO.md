...
- [x] `:frontend` モジュール（Kotlin/Wasm + Compose）と画面の枠
      ...
      - [ ] アカウント画面の中身を実データにする。フィードと記事は Phase 5、
            ...
            - 存在するアカウントかどうかは `Query.account` を引くようにした。
              無い名前は見つからない表示にする。名前・acct・Actor の URL は
              サーバーが返したものを出す
- [x] 日本語のフォントを配信して読み込む
...
- [ ] 管理 API を GraphQL にする（スキーマ優先で、間の型は生成する）
      ...
      - ログインが要らない口は `Query` の直下に置く（アカウント画面が引く `Query.account`）。
        引き当ては DataLoader を通す。仕組みは docs/architecture.md を参照
