...
- [x] `Follow` を記録してから `Accept` を投函する
      - [ ] `Accept` を投函した後の記録に何度も失敗した場合は諦めてログに残すだけ。
...
- [x] 配信キュー: 失敗時に指数バックオフでリトライ、上限到達で諦める
      ...
      - [ ] 送っている最中の投稿を消すと、相手には `Delete` の後に `Create` が届きうる。
            投函済みで未送信のものは投稿を消すと行ごと消えるので、残るのは送信中の分だけ
      - [ ] 投函の直前にアカウントを消して同じ名前で作り直すと、消しそこねた行が
            新しいアカウントとして配られうる。行はアカウントを名前で持っている
      - [x] `Accept{Follow}`・`Delete{Note}`・`Delete{Actor}`・`Update{Actor}` と
            フォロー成立後の過去の投稿もキューに載せる
            （種別は `kind`。仕組みは [docs/architecture.md](docs/architecture.md) の「配信キュー」）
