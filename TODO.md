...
現在地: Phase 0 は完了。`:backend` / `:backend:crypto` / `:backend:repository` / `:shared` /
`:frontend` の 5 モジュール構成。サーバー専用の crypto と repository は `backend/` の下に置き、
両方から使う shared は root に置いている。
...
管理画面のパスワードハッシュ。`nativeTest` で native バイナリ上でも動くことを確認済み。
...
`:backend:repository`（DB アクセス）、`:shared`（共有する型）、`:frontend`（管理 UI）の 5 つ。
crypto と repository は JVM のライブラリに依存していて `:frontend` からは使えないため、
`backend/` の下にネストしている。shared は両方から使うので root に置いている。
